...
class DeepQNetwork:
    ...
    def __learn(self):
        ...
        if GREEDY_POLICY_EPSILON < EPSILON_MAX:
        ...
        while True:
            ...
            while True:
                ...
                if MEMORY_COUNTER > MAX_MEMORY_CAPACITY:
                    self.__learn()
                    if is_game_over and not scoreout is None:
                        scoreout.write(
                            '{0} {1} {2}\n'.format(i_episode, episode_reward, env.seconds + env.ticks / env.fps))

                print('Episode:', i_episode, ' | Step: ', step, ' | Step Rewards: ', round(reward, 2))
                step += 1
                if step % 10000 == 0:
                    self.__save(step)
